...
export class AuthLoginQuery implements Query<UserLoginRequest, UserWithSessionInfoResponse> {
...
export class AuthUserInfoQuery implements Query<UserInfoRequest, UserInfoResponse> {
    ...
    async handle(request: UserInfoRequest): Promise<UserInfoResponse> {
        return await BackendApiCaller.fetchJson<UserInfoResponse>(`api/v1/auth/users`)
    }
}

export class AuthUserRegisterQuery implements Query<UserRegisterRequest, UserWithSessionInfoResponse> {
