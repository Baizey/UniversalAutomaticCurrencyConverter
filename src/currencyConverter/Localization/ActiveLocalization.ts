...
export class ActiveLocalization implements IActiveLocalization {
	...
	constructor( {
		             ...
		             localizationConfig,
	             }: ActiveLocalizationDep & CurrencyLocalizationDi ) {
		this.disabledCurrencies = currencyTagConfig.disabled
		this.logger = logger
		...
		this.krone =
			currencyLocalization.create( {
				key: kroneKey,
				setting: localizationConfig.krone,
			} )
		this.dollar =
			currencyLocalization.create( {
				key: dollarKey,
				setting: localizationConfig.dollar,
			} )
		this.yen =
			currencyLocalization.create( {
				key: yenKey,
				setting: localizationConfig.yen,
			} )
	}
	...
	determineForSite( siteAsText?: string ): void {
		const text = siteAsText || this.browser.document.body.textContent || ''
		// If the user has locked localization for site, do nothing
		if ( this.isLocked ) return
		const shared = Localizations.shared

		this.yen.setDetected(
			this.determineLocalization( this.yen.value, text, shared['¥'] ),
		)
		this.logger.debug(
		...
		this.dollar.setDetected(
			this.determineLocalization( this.dollar.value, text, shared['$'] ),
		)
		this.logger.debug(
		...
		this.krone.setDetected(
			this.determineLocalization( this.krone.value, text, shared['kr'] ),
		)
		this.logger.debug(
