...
export class BackendApi implements IBackendApi {
  ...
  async rate(from: string, to: string): Promise<ICurrencyRate | undefined> {
    ...
    if (!this._rates[from][to]) {
      const info = await this._browser.loadLocal<RateStorage | undefined>(
        rateKey
      );
    ...
    await this._browser.saveLocal(rateKey, {
      rate: rate.rate,
      timestamp: Date.now(),
      path: rate.path,
    } as RateStorage);
