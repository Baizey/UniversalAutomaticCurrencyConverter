...
export class TabState {
	...
	setIsShowingConversions( value: boolean ) {
		...
		if ( value ) {
			this.conversions.forEach( ( e ) => e.showConverted() )
		} else {
			this.conversions.forEach( ( e ) => e.showOriginal() )
		}
	}

	async updateDisplay( to?: string ): Promise<void> {
		if ( to ) {
			await this.conversions.forEach( ( e ) => e.convertTo( to ) )
		} else {
			await this.conversions.filter( ( e ) => e.updateDisplay() )
		}
	}
