...
const TextTemplate: Story = (args) => (
  <NormalText {...args}>{args.text}</NormalText>
);
const SmallTextTemplate: Story = (args) => (
  <FooterText {...args}>{args.text}</FooterText>
);
const BigTextTemplate: Story = (args) => <Title {...args}>{args.text}</Title>;
const LinkTextTemplate: Story = (args) => (
  <Link {...(args as LinkProps)}>{args.text}</Link>
);
...
export const link = LinkTextTemplate.bind({
  target: '',
  href: '',
} as LinkProps);
link.args = { text: 'Link' };
